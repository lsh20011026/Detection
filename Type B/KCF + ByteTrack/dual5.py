...
def mouse_callback(event, x, y, flags, param):
    global target_center, target_roi, fixed_roi_size, center_history, tracking_active, lost_frames, mode, frame_buffer, tracker, bbox_history
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if mode == 0:
            ...
            lost_frames = 0
            bbox_history = [(init_bbox[2], init_bbox[3])]
            
            if success:
                print(f"KCF 초기화 성공: {init_bbox}")
            else:
                print(f"KCF init 경고(s=False): {init_bbox} - 그래도 추적 시작")
            ...
            if results_click is not None and results_click.boxes is not None:
                ...
                if best_box is not None:
                    ...
                    bbox_history = [(bbox_w, bbox_h)]
                    fixed_roi_size = (int(bbox_w * roi_scale), int(bbox_h * roi_scale))
                    
                    tracking_active = True
                    lost_frames = 0
                    print(f"bbox:{bbox_w}x{bbox_h} -> ROI:{fixed_roi_size} 중심:{target_center}")
                    target_roi = None
                    return
...
cv2.namedWindow("YOLO + KCF Tracker (Scale Adaptive)", cv2.WINDOW_NORMAL)
cv2.setMouseCallback("YOLO + KCF Tracker (Scale Adaptive)", mouse_callback)

print("=== YOLO + KCF (Scale Adaptive ROI) ===")
...
while cap.isOpened():
    ...
    if tracking_active:
        if mode == KCF_MODE and tracker is not None:
            ...
            if success:
                ...
                bbox_history.append((bbox_w, bbox_h))
                ...
                print(f"KCF Scale: {avg_w}x{avg_h} -> ROI:{fixed_roi_size}")
            ...
            if best_box is not None:
                ...
                if abs(roi_cx - target_center[0]) <= 2 and abs(roi_cy - target_center[1]) <= 2:
                    cv2.putText(frame, "중심OK", (target_roi[0], target_roi[1]-50), 
    ...
    cv2.imshow("YOLO + KCF Tracker (Scale Adaptive)", annotated)
...
print("YOLO + KCF (Scale Adaptive ROI) 완료!")
