...
def mouse_callback(event, x, y, flags, param):
    global roi1, kcf_tracker, bytetrack_active, tracking_active, lost_frames, frame_buffer, bbox_history, center_history, fixed_roi_size, roi_adjust_frame_count
...
print("ROI_SCALE=1.5 | Click:Track | B:Toggle | R:Reset | Q:Quit")
...
while cap.isOpened():
    ...
    if tracking_active and kcf_tracker is not None:
        ...
        if roi_adjust_frame_count >= roi_adjust_interval:
            ...
            if roi1 is not None and bytetrack_active:
                ...
                roi_crop = frame[y1:y1+h, x1:x1+w]
                if roi_crop.size > 0:
                    high_conf_results = model.track(roi_crop, persist=True, tracker="bytetrack.yaml", conf=HIGH_CONF_THRESHOLD)[0]
                    ...
                    if high_conf_results is not None and high_conf_results.boxes is not None:
                        ...
                        if best_high_conf_box is not None:
                            ...
                            print(f"ROI1 Adjust: {new_roi1} (conf:{max_conf:.2f}, obj_c:{int(new_cx)},{int(new_cy)}) {'✅' if kcf_success else '⚠️'}")

    if tracking_active and roi1 is not None:
        if not target_found:
            cv2.rectangle(frame, (roi1[0], roi1[1]), (roi1[0]+roi1[2], roi1[1]+roi1[3]), (0, 165, 255), 2)
            cv2.putText(frame, f"Lost: {lost_frames}", (roi1[0], roi1[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
    ...
    if roi1:
        ...
        cv2.putText(frame, f"Tgt:{rw}x{rh} Scl:{ROI_SCALE}", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
        cv2.putText(frame, f"Adj:{roi_adjust_frame_count}/{roi_adjust_interval}", (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 165, 0), 2)
    
    cv2.putText(frame, "Click:BTrack | B:Toggle | R:Reset | Q:Quit", (10, 470), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
...
print("ROI1=객체중심*1.5 | 테스트: 확대/축소 확인!")
